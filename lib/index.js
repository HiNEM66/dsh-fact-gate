...
import { scanCompactionStarts, COMPACTION_NOTICE } from "./compaction.js";
import { isGitPushCommand, isGitPushCommandLax, PUSH_REVIEW_PROMPT, PUSH_REVIEW_SCHEMA, formatReviewMessage } from "./push-review.js";
...
export const apply = (ctx, config) => {
        ...
        try {
            ...
            try {
                ...
                if (settled.structured !== undefined) {
                    return formatReviewMessage(settled.structured);
                }
    ...
    ctx.on('tools/code-dispatch-log', (dispatch, next) => {
        if (dispatch.name !== 'pwsh' && dispatch.name !== 'bash')
            return next();
        // 命令载体（真机实证 code-mode.ts:505）：CodeDispatchLog.exec 是外层
        // run_code 执行，exec.arguments 是 {code, description}；内层子调用的
        // command 不暴露给事件，命令文本以内层调用形式存在于 code 程序里
        // （tools.pwsh({ command: `git ${...} push` })）。模板变量无法静态
        // 展开 → 宽松按行匹配；误报由 maybePushReview 的 `->` 成功标志兜底。
        const args = dispatch.exec.arguments;
        const direct = typeof args?.command === 'string' ? args.command : '';
        const code = typeof args?.code === 'string' ? args.code : '';
        if (!isGitPushCommand(direct) && !isGitPushCommandLax(code))
            return next();
    ...
    function attachAgent(agent) {
        ...
        // Compaction notice via agent/pre-step + agent/turn-stopping + session
        // event scan: compaction/start is appended to the session event stream
        // (compaction-basic region.ts:189), not emitted on a cordis event a plugin
        // fiber can listen to; the automatic compaction backend itself triggers
        // inside an agent/pre-step listener (compaction-basic index.ts:147). Scan
        // on BOTH pre-step and turn-stopping: a manual /compact turn ends without
        // a following step, so pre-step alone never fires after it (real-machine
        // session "测试5": compaction/start..end present, zero steps after).
        let lastCompactionScanSeq = agentView.session.events.length > 0
            ? agentView.session.events[agentView.session.events.length - 1].seq ?? 0
            : 0;
        const scanAndNotifyCompaction = () => {
        ...
        // turn-stopping covers a manual /compact whose turn ends without a next step.
        agentView.ctx.on('agent/turn-stopping', () => {
            scanAndNotifyCompaction();
        });
    ...
    // Resumed sessions do NOT re-emit agent/created (announce() only fires on
    // register(); resumed agents are inserted without announce) — and the
    // apply-time ctx.get('agents').list() sweep only covers agents that are
    // live at boot. A session resumed later (e.g. the web profile's persisted
    // session picked up after a restart) therefore never reaches attachAgent
    // through either path (real-machine: /compact ran cleanly but no notice
    // was injected). agent/status fires on every lifecycle transition
    ...
    if (timer && typeof timer.setInterval === 'function') {
        timer.setInterval(() => {
            for (const scan of compactionScanners.values())
                scan();
        }, 30_000);
    }
