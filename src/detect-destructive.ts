...
function escapeRegexMeta(source: string): string {
 ...
 */
export function explodeSubshells(input: string): string {
  let out = input;
  for (let i = 0; i < 4; i += 1) {
    const before = out;
    out = out.replace(/\$\(([^()`]*)\)/g, ';$1;');
 ...
 */
function tokenizeAllowlistedShellWords(input: string): string[] | null {
  const tokens: string[] = [];
  let current = '';
 ...
 */
function quoteAwareSegments(input: string): string[][] {
...
function findGitSubcommand(tokens: string[]): { command: string; rest: string[] } | null {
 ...
 */
function collectExecutableBodies(raw: string): string[] {
  const bodies = [raw];
  const queue = [raw];
...
function isDestructiveFindExec(command: string): boolean {
 ...
 */
function isDestructiveQuoteAware(raw: string, depth = 0, config: DetectorConfig): boolean {
...
export function isReadOnlyGitIntrospection(command: string): boolean {
...
export function compileExemptGlobs(globs: string[]): RegExp[] {
  const out: RegExp[] = [];
...
export function isExemptPath(filePath: string, matchers: RegExp[]): boolean {
...
export function isDestructiveBash(command: string, config: DetectorConfig): boolean {
  const raw = String(command || '');
  ...
  if (extra.some(re => re.test(flattened))) return true;
  ...
  for (const segment of segments) {
    ...
    if (extra.some(re => re.test(stripped))) return true;
  ...
  if (isDestructiveQuoteAware(raw, 0, config)) return true;

  return false;
}

/** Compile operator extra destructive regexes (case-insensitive, fail-open). */
export function compileExtraRegexes(sources: string[]): RegExp[] {
  const out: RegExp[] = [];
