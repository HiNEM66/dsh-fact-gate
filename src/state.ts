...
const EMPTY_STATE = (): FactGateState => ({ checked: [], last_active: Date.now(), fact_force_denials: 0 });
...
export function hashSessionKey(prefix: string, value: string): string {
...
export class FactGateStateStore {
  ...
  private stateFileFor(sessionKey: string): string {
  ...
  load(sessionKey: string): FactGateState {
    ...
    try {
      if (existsSync(stateFile)) {
        const state = JSON.parse(readFileSync(stateFile, 'utf8')) as Partial<FactGateState>;
  ...
  save(sessionKey: string, state: FactGateState): boolean {
    ...
    try {
      ...
      try {
        if (existsSync(stateFile)) {
          const diskState = JSON.parse(readFileSync(stateFile, 'utf8')) as Partial<FactGateState>;
      ...
      const finalState: FactGateState = {
      ...
      try {
        ...
        if (error && (error as NodeJS.ErrnoException).code === 'EEXIST' || (error as NodeJS.ErrnoException).code === 'EPERM') {
          try {
            unlinkSync(stateFile);
          } catch (_) {
            /* ignore */
  ...
  markCheckedAndCountDenial(sessionKey: string, key: string): { ok: boolean; denials: number } {
    const state = this.load(sessionKey);
    if (!state.checked.includes(key)) {
      state.checked.push(key);
    }
...
export function getDenialCount(state: Partial<FactGateState> | null | undefined): number {
  const n = Number(state && state.fact_force_denials);
  return Number.isFinite(n) && n >= 0 ? Math.floor(n) : 0;
}

export function getFullDenialBudget(envOverride?: string): number {
...
function pruneCheckedEntries(checked: string[]): string[] {
...
export function resolveSessionKey(agentId: string | undefined, env: { FACT_GATE_SESSION_ID?: string; FACT_GATE_PROJECT_DIR?: string }): string {
