...
export class DuplicateReadTracker {
  ...
  constructor(private config: () => DuplicateReadConfig) {}
  ...
  recordRead(sessionKey: string, filePath: string): { duplicate: boolean; path: string } {
    ...
    if (this.config().enabled && filePath) {
      ...
      try {
        ...
        const perSession = this.hashes.get(sessionKey) ?? new Map<string, string>();
  ...
  private detectDuplicate(sessionKey: string, filePath: string): boolean {
    if (!this.config().enabled || !filePath) return false;
    const abs = resolve(filePath);
