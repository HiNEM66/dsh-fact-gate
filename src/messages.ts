...
export function writeGateMsg(filePath: string): string {
  const safe = sanitizePath(filePath);
  return [
    '[Fact-Forcing Gate]',
    '',
 ...
 */
export function condensedGateMsg(action: string, filePath: string, ordinal: number): string {
  const safe = sanitizePath(filePath);
  return (
...
export function destructiveBashMsg(): string {
  return [
    '[Fact-Forcing Gate]',
    '',
...
export function routineBashMsg(): string {
  return [
    '[Fact-Forcing Gate]',
    '',
...
export function withRecoveryHint(message: string, hookIds: string[] = [EDIT_WRITE_HOOK_ID]): string {
